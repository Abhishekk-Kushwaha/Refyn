...
## 11. EXAM-AGNOSTIC UI RULES (non-negotiable)

The database (Doc 2, Section 15) is built so adding SSC, Banking, or GMAT later is a **data problem** — inserting rows — not a code problem. That promise only holds if every screen follows these rules while being built. Skip one while rushing through a screen, and "just add rows" quietly turns into "also go hunt down hardcoded CAT references across a dozen components."

**Rule 1 — No exam name hardcoded in copy.**
```jsx
// ❌ WRONG
<h2>Where are you in your CAT journey?</h2>

// ✅ RIGHT
<h2>Where are you in your {activeExam.name} journey?</h2>
```
Brand-level copy that doesn't reference a specific exam ("Hunt your weakness. Own the exam.") is fine as static text — it's already exam-agnostic by design.

**Rule 2 — Onboarding must include a real exam-selection step.**
The current onboarding flow implicitly assumes CAT. Before the "journey/weak areas/daily target" steps, add a **Step 0: exam selection** — reads from the `exams` table (`is_active = true`), writes the choice to `user_exams`. Every later step, and the rest of the app, reads the selected exam from there. Nothing downstream should assume CAT by default.

**Rule 3 — Charts render dynamically off however many topics exist.**
The dashboard radar chart is currently modeled around CAT's 5 topics. It must render N axes based on the active exam's actual `topics` rows — not a hardcoded 5- or 6-axis shape. If SSC ends up with a different topic count or balance, the same component handles it without a code change.

**Rule 4 — Every data-fetching call requires `examId`, no exceptions.**
Per Database doc Section 15.2 — service functions take `examId` as a required (non-optional) parameter. This is enforced at the TypeScript level: `examId: string`, never `examId?: string`. A missing exam context should fail to compile, not silently fetch the wrong exam's content.

**Rule 5 — `examStore` (Zustand) is the single source of truth for "which exam is active right now."**
No component reads exam context from anywhere else — not from a prop drilled from three levels up, not re-derived from the URL. One store, one read path. This is what eventually makes an exam-switcher (for someone prepping two exams) a small addition instead of a refactor.

**Rule 6 — Exam-specific phrasing pulls from data, not literals.**
"Appeared in CAT 22, CAT 21, CAT 19" on a flashcard back becomes "Appeared in {examShortName} {year}" for each year in `years_appeared`. Small detail, same principle as Rule 1 — anywhere an exam name could appear in generated or templated text, it comes from the active exam record.

**How to self-check while building a screen:** search the component for any literal string containing "CAT" that isn't part of a fixed brand phrase. If one exists and it's describing exam-specific content (not the Refyn brand tagline), it's a violation of these rules and needs to read from `activeExam` instead.
